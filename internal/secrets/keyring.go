...
// it's missing — a pure read used by fetchOrCreateDEK's fast path (dek.go)
// to decide whether cross-process bootstrap locking is needed at all.
// Bypasses the in-process memoization layer, same as fetchOrCreateKEK.
func peekKEK(profileID string) (kek []byte, found bool, err error) {
	keyringIOMu.Lock()
	defer keyringIOMu.Unlock()

	stored, err := keyring.Get(keyringService, kekAccount(profileID))
	if err != nil {
...
func fetchOrCreateKEK(profileID string) ([]byte, error) {
	...
	stored, err := keyring.Get(keyringService, account)
	if err == nil {
		key, decodeErr := hex.DecodeString(stored)
		if decodeErr != nil {
	...
	if err := keyring.Set(keyringService, account, hex.EncodeToString(kek)); err != nil {
