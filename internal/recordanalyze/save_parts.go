...
func saveFragment(reg Installer, stage, id, from, name string) (*SaveResult, error) {
	...
	// A colliding name gets a suffix only when that fragment differs; an
	// identical one keeps the name and AddFragment reports no changes.
	frag := action.FragmentDef{Description: def.Description, Inputs: def.Inputs, Steps: def.Steps}
	name = Unique(name, "_", func(n string) bool {
		...
		existing, err := target.Fragment(n)
		if err != nil {
			return false
		}
		mine := frag
		mine.Name = n
		return canonicalJSON(existing) != canonicalJSON(&mine)
	})
	frag.Name = name
...
// canonicalJSON renders v with sorted keys and no formatting differences
// (embedded raw JSON is re-encoded too), for content comparison.
