...
(`tool-fence.ts`), which drops every key not listed. So every call ran with `input: {}`, whatever
the role passed. In the first run the templated path rendered as `<no value>`: reads failed with
