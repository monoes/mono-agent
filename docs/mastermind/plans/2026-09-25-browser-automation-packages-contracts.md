...
| **wiring** (CLI + node/runtime wiring) | `cmd/monoagentcli/automation*.go` (new), `cmd/monoagentcli/action_template.go`, `cmd/monoagentcli/action_export.go` (new), `cmd/monoagentcli/login.go`, `cmd/monoagentcli/root.go`, `internal/nodes/browser_register.go`, `internal/nodes/browser_adapter.go`, `internal/browser/provider.go`, `internal/workflow/schema_loader.go`, `internal/connections/manager.go`, startup wiring (wherever `SetGlobalSessionProvider` is called) | `internal/automation/**` |
...
| **ingest** (recording storage) | `internal/recording/**`, `internal/extension/{protocol.go,server.go,recording.go}` (routing of `kind:"recording"` frames), `internal/capture/meta.go` (add source "recording"), `cmd/monoagentcli/record.go` (list/show/delete) | `cmd/monoagentcli/record_analyze*.go` |
| **analyze** (AI + verify + save) | `internal/recordanalyze/**`, `data/skills/record-to-action.md`, `cmd/monoagentcli/record_analyze*.go` (analyze/verify/save subcommands, attached to the `record` command through an exported `func AddRecordAnalyzeCommands(parent *cobra.Command)` pattern — coordinate by name only) | `internal/recording/**` |
