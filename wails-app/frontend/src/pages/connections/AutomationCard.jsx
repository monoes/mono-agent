...
import { Plus, Circle, FileCode, AlertTriangle, ArrowUpCircle, PenLine } from 'lucide-react'
...
function loginText(s) {
  ...
  if (s?.username) return 'session expired'
  return 'logged out'
}
...
export default function AutomationCard({ automation: a, onOpen }) {
  ...
  const disabled = !unavailable && a.enabled === false
  return (
    <button
      type="button"
      onClick={onOpen}
      ...
      aria-label={`${a.name || a.id}, ${loginText(s)}, ${a.actions} actions`}
      style={{ ...cardStyle(s.loggedIn && !unavailable, hov), opacity: unavailable || disabled ? 0.75 : 1 }}
    >
